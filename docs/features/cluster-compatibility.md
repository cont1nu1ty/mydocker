...
让未来的分布式工作负载平台能够通过带版本的 `mydockerd` API 调度、分配、调谐和
恢复工作负载，同时保持单节点所有权、幂等性，以及可单独测量的运行时延迟。
...
| `Task` | 稳定的期望工作负载或副本身份 |
| `TaskSpec` | 镜像摘要、命令/环境、资源、策略和 generation |
...
flowchart TD
    ...
    Agent -->|versioned local API| Daemon[mydockerd]
...
节点本地运行时延迟与 control-plane 延迟分别报告。不能只报告总耗时，
因为总耗时无法定位瓶颈。
...
只有在某个明确的 2.0 tag 或 commit 已具备 Sandbox/Attempt 生命周期、本地 API、
daemon 恢复测试和节点本地基线后，才可以创建集群分支。运行时/公共 API 修复先落入
`mydocker-2.0`，再集成到集群；集群专有的 control-plane 变更不回流到运行时。
...
- 初始 control-plane API/store 技术和一致性契约。
